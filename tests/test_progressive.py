...
def test_train_vs_eval_maxcut():
    ...
    train_inputs = 0 * z_stars[:N_train, :]
    test_inputs = 0 * z_stars[N_train:, :]
    ...
    # initialize from zero
    z0 = jnp.zeros((N_train, m + n))
    _, out_from_zero, __ = lasco_scs_model.static_eval(10, z0, q_mat_train, z_stars_train, 0)
    z1_from_zero = out_from_zero[2][:, 1, :-1]
    z2_from_zero = out_from_zero[2][:, 2, :-1]
    z10_from_zero = out_from_zero[2][:, 10, :-1]
    
    # initialize from z1_from_zero
    _, out_from_old_z1, __ = lasco_scs_model.static_eval(10, z1_from_zero, q_mat_train, z_stars_train, 0)
    z1_from_old_z1 =  out_from_old_z1[2][:, 1, :-1]
    assert(jnp.linalg.norm(z2_from_zero - z1_from_old_z1) < 1e-8)

    # evaluate with the train method
    out_train = lasco_scs_model.loss_fn_train(lasco_scs_model.params,
                                             z0, 
                                             q_mat_train,
                                             10, 
                                             z_stars_train,
    ...
    # train on the first 5 steps
    num_iters = 100
    params, state = [lasco_scs_model.params[0][:5, :]], lasco_scs_model.state
    batch_indices = jnp.arange(N_train)
    losses = np.zeros(num_iters)
    inputs = z0
    for i in range(num_iters):
        loss, params, state = lasco_scs_model.train_batch(batch_indices, inputs, params, state) 
        losses[i] = loss
    pp = lasco_scs_model.params[0]
    pp = pp.at[:5, :].set(params[0])
    lasco_scs_model.params = [pp]
    lasco_scs_model.state = state
    ...
    z1_from_zero = out_from_zero[2][:, 1, :-1]
    z2_from_zero = out_from_zero[2][:, 2, :-1]
    z5_from_zero = out_from_zero[2][:, 5, :-1]
    z10_from_zero = out_from_zero[2][:, 10, :-1]
    out_train = lasco_scs_model.loss_fn_train(lasco_scs_model.params,
                                             z0, 
                                             q_mat_train,
                                             ...
                                             0)
    assert(jnp.abs(out_train - jnp.linalg.norm(z5_from_zero - z_stars_train, axis=1).mean()) < 1e-8)
    old_params = lasco_scs_model.params[0]

    # shift the params and evaluate from z1
    lasco_scs_model.params = [lasco_scs_model.params[0][1:, :]]
    _, out_from_z1, __ = lasco_scs_model.static_eval(10, z1_from_zero, q_mat_train, z_stars_train, 0)
    z1_1_from_z1 = out_from_z1[2][:, 1, :-1]
    import pdb
    pdb.set_trace()
    assert(jnp.linalg.norm(z1_1_from_z1 - z2_from_zero) < 1e-8)
