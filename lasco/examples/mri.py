...
import os
from lasco.examples.solve_script import direct_osqp_setup_script
...
def setup_probs(setup_cfg):
    ...
    P, A, prob, img_param, num_eq = mnist_canon(img_size, img_size, lambd, lambd2=0)
    m, n = A.shape

    # create theta_mat
    noise_matrix = cfg['noise_std_dev'] * np.array(np.random.normal(size=(N, img_size * img_size)))
    # theta_mat = np.clip((B @ x_train[:N, :].T).T + noise_matrix, a_min=0, a_max=1)
    theta_mat = np.clip((x_train[:N, :].T).T + noise_matrix, a_min=0, a_max=1)

    q_mat = get_q_mat(theta_mat, prob, img_param, m, n, img_size)
    ...
    for i in range(5):
        ...
        num_x = P.shape[0]
        sol_img = np.reshape(z_stars[i, num_x - 2 * img_size ** 2: num_x - img_size ** 2], (img_size, img_size))
...
def get_q_mat(img_matrix, prob, img_param, m, n, img_size):
    """
    ...
    q_mat = q_mat.at[:, n + img_size ** 2:n + m].set(-jnp.inf)
    q_mat = q_mat.at[:, n + m + img_size ** 2:].set(g)

    q_mat = q_mat.at[:, n:n + img_size ** 2].set((img_matrix.T).T)
    q_mat = q_mat.at[:, n + m:n + m + img_size ** 2].set((img_matrix.T).T)
...
def mnist_canon(m_img, n_img, lambd, lambd2=0):#, blurred_img):
    # create cvxpy prob
    # b = np.ravel(blurred_img)
    img_param = cp.Parameter(m_img * n_img)
    # m_img, n_img = 28, 28
    ...
    prob = cp.Problem(cp.Minimize(lambd * tv(x) + lambd2 * cp.sum(x) + cp.sum_squares(cp.vec(x) - img_param)), constraints)
    # data, chain, inverse_data = prob.get_problem_data(cp.OSQP)

    img_param.value = 3.3 * np.random.rand(m_img * n_img)
    soln = prob.solve()
    data, chain, inverse_data = prob.get_problem_data(cp.OSQP)
...
def plot_mnist_img(x_star, blurred_img_vec, x_no_learn, x_nn, x_learn, filename, figsize=784):
    ...
    axarr[3].set_title('nearest neighbor')
    axarr[3].axis('off')
