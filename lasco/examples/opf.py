...
from lasco.algo_steps import create_M
from scipy.sparse import csc_matrix
from lasco.examples.solve_script import setup_script
from lasco.launcher import Workspace
from lasco.algo_steps import get_scaled_vec_and_factor
from lasco.examples.opf_data.case14 import case14
...
def generate_graph_and_prob(ppc, d_vertices, d_bounds, seed=42):
    ...
    X = cp.Variable((n, n), symmetric=True) #, hermitian=True)
    ...
    constraints = [
        X >> 0,
        cp.diag(X) >= np.square(vmin_arr),
        cp.diag(X) <= np.square(vmax_arr),
    ]
...
def static_canon(ppc, d_vertices, d_bounds, rho_x=1, scale=1, factor=True, seed=42):
    ...
    d_param.value = np.array(d_bounds)
    ...
    if factor:
        rho_y_scale = 1
        algo_factor, scale_vec = get_scaled_vec_and_factor(M, rho_x, scale, rho_y_scale, m, n,
                                                           zero_cone_size)
