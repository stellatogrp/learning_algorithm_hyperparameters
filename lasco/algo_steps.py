...
def k_steps_eval_lm_scs(k, z0, q, params, proj, P, A, supervised, z_star, jit, hsde, zero_cone_size,
    ...
    if hsde:
        ...
        homogeneous = False
        z_next, u, u_tilde, v = fixed_point_hsde(
            z0, homogeneous, q, factor1, factor2, proj, scale_vec, 1, tau_factor, lasco=True, verbose=verbose)
        all_z = all_z.at[1, :].set(z_next)
        all_u = all_u.at[1, :].set(u)
        all_v = all_v.at[1, :].set(v)
        iter_losses = iter_losses.at[0].set(jnp.linalg.norm(z_next - z0))
...
def fp_eval_lm_scs(i, val, q_r, z_star, factor, proj, P, A, c, b, hsde, homogeneous, 
    ...
    factor1, factor2 = factor
    z_next, u, u_tilde, v = fixed_point_hsde(
        z, homogeneous, r, factor1, factor2, proj, scale_vec, 1, tau_factor, lasco=True,
        verbose=verbose)
...
def k_steps_train_lm_scs(k, z0, q, params, P, A, supervised, z_star, proj, jit, hsde):
    ...
    if hsde:
        ...
        z_next, u, u_tilde, v = fixed_point_hsde(
            ...
            factor2, proj, scale_vec, 1, tau_factor, lasco=True)
        iter_losses = iter_losses.at[0].set(jnp.linalg.norm(z_next - z0))
        z0 = z_next
    val = z0, iter_losses
    start_iter = 1 if hsde else 0
...
def k_steps_eval_lasco_osqp(k, z0, q, params, P, A, supervised, z_star, jit, custom_loss=None):
    ...
    fp_eval_partial = partial(fp_eval_lasco_osqp,
                              supervised=supervised,
                              z_star=z_star,
...
def k_steps_train_lasco_osqp(k, z0, q, params, A, supervised, z_star, jit):
    ...
    fp_train_partial = partial(fp_train_lasco_osqp,
                               supervised=supervised,
                               z_star=z_star,
                               all_factors=all_factors,
                               A=A,
...
def fp_eval_lasco_osqp(i, val, supervised, z_star, all_factors, P, A, q, rhos, sigmas, alphas,
                       custom_loss=None, lightweight=False):
    m, n = A.shape
...
def fp_train_scs(i, val, q_r, factor, supervised, z_star, proj, hsde, homogeneous, scale_vec, alpha):
    ...
    if hsde:
        r = q_r
        z_next, u, u_tilde, v = fixed_point_hsde(z, homogeneous, r, factor[0], factor[1], proj, scale_vec, alpha, TAU_FACTOR, lasco=False)
    else:
        q = q_r
...
def fp_eval_scs(i, val, q_r, z_star, factor, proj, P, A, c, b, hsde, homogeneous, scale_vec, alpha,
    ...
    if hsde:
        r = q_r
        z_next, u, u_tilde, v = fixed_point_hsde(
            z, homogeneous, r, factor[0], factor[1], proj, scale_vec, alpha, TAU_FACTOR, lasco=False, verbose=verbose)
    else:
        q = q_r
...
def k_steps_train_scs(k, z0, q, factor, supervised, z_star, proj, jit, hsde, m, n, zero_cone_size,
    ...
    if hsde:
        ...
        homogeneous = False
        z_next, u, u_tilde, v = fixed_point_hsde(
            z0, homogeneous, q, factor[0], factor[1], proj, scale_vec, alpha, TAU_FACTOR, lasco=False)
        iter_losses = iter_losses.at[0].set(jnp.linalg.norm(z_next - z0))
        z0 = z_next
    val = z0, iter_losses
    start_iter = 1 if hsde else 0
...
def k_steps_eval_scs(k, z0, q, factor, proj, P, A, supervised, z_star, jit, hsde, zero_cone_size,
    ...
    if hsde:
        ...
        z_next, u, u_tilde, v = fixed_point_hsde(
            z0, homogeneous, q, factor[0], factor[1], proj, scale_vec, alpha, TAU_FACTOR, lasco=False, verbose=verbose)
...
def fixed_point_hsde(z_init, homogeneous, r, factor1, factor2, proj, scale_vec, alpha, tau_factor, lasco=True, verbose=False):
    """
